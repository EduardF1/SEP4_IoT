...
static void setupRC_SERVO()
{
	...
	rc_servo_setPosition(0, -100);	//	reset the driver upon setup
}

//	Function to create the RC_SERVO task externally
void createRC_SERVOTask(QueueHandle_t rc_servo_queue, SemaphoreHandle_t mutexSemphr)
{
	...
	_mutexSemphr = mutexSemphr;	//	pass the given(argument) semaphore to the field semaphore
